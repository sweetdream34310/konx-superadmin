...
import { FORGOT_PASSWORD, LOGIN_NOW, SOFTWARE_NAME, Submit, YOUR_EMAIL } from "../constant";
  ...
  const handleClick = () => {
    if (email == "" ) {
      ...
      if (!email.match(emailValidRegex)) {
        ...
        const resForgotPassword = ForgotPasswordApi(req);

        if (resForgotPassword) {
            toast.success('Send new password to your email.')
            Navigate("/auth/login");
        }
